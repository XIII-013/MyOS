...
void map_pagetable(pagetable_t pgtbl) {
...
int kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz) {
  ...
  for(i = PGROUNDUP(start);i < start + sz;i += PGSIZE) {
    if((pte = walk(src, i, 0)) == 0) panic("pte should exist");
    if((*pte & PTE_V) == 0) panic("page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & ~PTE_U; // 将该页设置为非用户页
